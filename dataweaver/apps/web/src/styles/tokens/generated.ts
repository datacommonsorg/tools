// ⚠️ AUTO-GENERATED — Do not edit. Modify the JSON tokens in 'packages/tokens/src/' and run 'pnpm generate:tokens'

import type { BezierDefinition } from "motion/react";

export const BREAKPOINT_TABLET = 768;
export const BREAKPOINT_LAPTOP = 1280;
export const BREAKPOINT_DESKTOP = 1600;

export const COLORS = {
	black: "0, 0, 0",
	"grey-almost-black": "32, 33, 36",
	"grey-muted": "68, 71, 70",
	"grey-light": "227, 231, 237",
	white: "255, 255, 255",
	blue: "11, 87, 208",
	"blue-light": "247, 252, 255",
} as const;

export const EASE_LINEAR: BezierDefinition = [0, 0, 1, 1];
export const EASE_OUT: BezierDefinition = [0.26, 1, 0.48, 1];
export const EASE_IN: BezierDefinition = [0.52, 0, 0.74, 0];
export const EASE_IN_OUT: BezierDefinition = [0.76, 0, 0.24, 1];
