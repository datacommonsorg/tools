...
- **places**: a JSON array of specific place names or DCIDs (resolve acronyms like "BRIC" → ["Brazil", "Russia", "India", "China"]). For sub-region or distribution queries (e.g. "counties in California", "California counties", "cities in Texas", "crime across the US", "countries across Africa"), extract the parent region/place (e.g. "California", "Texas", "United States", "Africa"). If the query is a follow-up, extract places from the full conversation context (original query + clarifications). Only return an empty array if no places are mentioned anywhere in the conversation.
