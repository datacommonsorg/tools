...
2. **Sub-Region / Child-Place Queries**: A query for a target place is a Child-Place Query if it meets EITHER of these conditions:
   - **Condition A (Explicit Sub-unit Noun)**: The query explicitly mentions a sub-division / granularity noun qualifying or dividing the target place (e.g. "counties in California", "California counties", "median household income for counties in California", "cities in Texas", "by county in California", "county-level data in California"). Set `childPlaceType` to the normalized type (e.g. "County", "City", "State", "Country").
   - **Condition B (Distribution Operator — "across", "throughout", "within")**: The query uses a distribution operator over the target place without specifying a child type (e.g. "income across California", "GDP across Europe", "poverty throughout the US"). You MUST infer the default canonical childPlaceType based on the parent's hierarchy:
...
0. **CRITICAL: NEVER INFER, PARAPHRASE, OR INVENT STATISTICAL VARIABLES**:
   ...
   - **For Sub-Region / Child-Place Queries** (e.g. "counties in California", "California counties", "income across California", "countries in Africa", "states in USA"): Do NOT use `search_indicators` with the continent/parent alone. You MUST use `search_child_indicators` (passing `sample_child_places` like `["country/NGA", "country/ZAF", "country/EGY"]`) and `get_child_observations` (passing `parent_place_dcid` and `child_place_type`, e.g. `parent_place_dcid: "africa"`, `child_place_type: "Country"`, `date: "all"`). Set `isChildQuery: true`, `parentPlaceDcid`, and `childPlaceType` on the returned variable.
...
8. **Always Pass Target Place to Search**:
   ...
   - When calling `search_child_indicators`, pass `parent_place` (e.g. `"africa"`) and representative `sample_child_places` (e.g. `["country/NGA", "country/ZAF", "country/EGY"]`).
9. **Verify Data Availability & Per-Variable Place Binding**: Set each variable's `placeDcid` and `placeName` to the verified place. For child queries, set `placeDcid` and `parentPlaceDcid` to the parent region DCID (e.g. `"africa"`), `isChildQuery: true`, and `childPlaceType` (e.g. `"Country"`). If you fell back to `Earth` because a target location lacked data, set `placeDcid` to `"Earth"`, `placeName` to `"World"`, and explain the fallback in `coverage`.
