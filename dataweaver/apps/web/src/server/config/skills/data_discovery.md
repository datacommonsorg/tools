...
2. **Sub-Region / Child-Place Queries** (e.g. US states, countries in Africa, counties in California): Do NOT use `search_indicators` or `get_observations`. Call `search_child_indicators` (with `parent_place` and `sample_child_places` to test if indicators exist), inspect metadata with `get_variable_metadata`, and fetch data across all children with `get_child_observations` (passing `parent_place_dcid` and `child_place_type`, e.g. `parent_place_dcid: "africa"`, `child_place_type: "Country"`, `date: "all"`). For child queries, set `"isChildQuery": true`, `"parentPlaceDcid": "<parent_dcid>"`, and `"childPlaceType": "<type>"` on each variable in the `variables` array.
...
0. **CRITICAL: NEVER INFER, PARAPHRASE, OR INVENT STATISTICAL VARIABLES**:
   ...
   - **For Sub-Region / Child-Place Queries** (e.g. "countries in Africa", "states in USA", "counties in California"): Do NOT use `search_indicators` with the continent/parent alone. You MUST use `search_child_indicators` (passing `sample_child_places` like `["country/NGA", "country/ZAF", "country/EGY"]`) and `get_child_observations` (passing `parent_place_dcid` and `child_place_type`, e.g. `parent_place_dcid: "africa"`, `child_place_type: "Country"`, `date: "all"`). Set `isChildQuery: true`, `parentPlaceDcid`, and `childPlaceType` on the returned variable.
