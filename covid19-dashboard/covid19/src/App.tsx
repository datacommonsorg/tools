  ...
  render = () => {
    ...
    const cumulativePanelColumns = Content.cumulativePanel.map(
      ({
        ...
        const values = filteredPlaces.map(place => {
          ...
          const latestDate = getMinMaxDate(dates);
    ...
    const dates = places
      .map(place => {
        ...
        return getMinMaxDate(dates, 'max');
      })
      .filter(date => date);
    ...
    const maxDate = getMinMaxDate(dates, 'max');
    ...
    return (
      <div>
        ...
        <div className={'site-container fadeIn'}>
          ...
          <CountPanel textToValue={cumulativePanelColumns} />
