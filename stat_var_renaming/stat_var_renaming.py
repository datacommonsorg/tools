...
def generate_dependent_constraint_list():
    ...
    for spec in obs_spec_list.spec:
        ...
        # Get dependent variables which depend on the value of the constraint.
...
def build_original_constraints(row):
    ...
    while (current_constraint <= _MAX_CONSTRAINTS and
            not pd.isna(row[f"orig_p{current_constraint}"])):
...
def output_stat_var_documentation(stat_vars):
    ...
    with open('statistical_variables.md', 'w', newline='') as f_out:
        ...
        for sv_grouping in STAT_VAR_POPULATION_GROUPINGS:
            f_out.write(DOCUMENTATION_HEADER_START
                    .replace("{HEADER}", sv_grouping.vertical))
            ...
            for population_type in sv_grouping.popTypes:
                ...
                group_pop_type = (sv_grouping.subgroupAllPops or
                                (sv_grouping.subgroupIfMoreThanOne and
                                    len(stat_vars_for_pop_type) > 1))
