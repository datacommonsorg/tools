...
Commons Knowledge Graph. These StatVar names are incredible useful for end-users
as they may be pulled from both the python API or Google Sheets API by name.
...
3) Certain data sets are blacklisted For example, all bio data sets and a few
   miscellaneous ones are excluded. This blacklist was created by Tiffany.
...
4) Dependent variables are removed. Dependent variables are constraints that are
   ...
   an income of 10k to 15k USD may only be measured by the census if they are
...
4) Boolean constraints are replaced by their populations E.g. p1 =
   isInternetUser and v1=True/False becomes v1=isInternetUser/notInternetUser.

5) Measurement properties are stripped from constraints. E.g. p1 = employment
   and v1 = USC_Unemployed becomes v1=Unemployed
...
7) Cause of death properties are renamed. E.g. p1 = causeOfDeath and
   ...
   names are generated directly from the ICD10 names stored in BigQuery. For
   exceptionally long or confusing names, I manually renamed them.

8) DEA drug names are renamed E.g. p1="drugPrescribed" and v1="drug/dea/9250"
   becomes v1="Methadone". These are manually renamed. Some drug names are
...
9) Certain variables have text pre/postpended to their constraints to improve
   readability. For example p1 = childSchoolEnrollment and
   v1=EnrolledInPublicSchool is changed to v1="ChildEnrolledInPublicSchool".
   These mappings are applied to ~15 variables.

10) Misc. changes a) MeasuredProp InsuredUnemploymentRate changed to
...
# Constants Max total number of constraints of a variable to include (Depedent
# variables excluded).
MAX_CONSTRAINTS = 3
...
def authenticate_bq_client():
  ...
  """
  # TODO Choose your authentication method.
...
def download_stat_vars(client):
  ...
  # Dynamically create list of blacklisted provences.
  blacklist = ['"%s"' % prov_id for prov_id in frozenset().union(
      *[_MISC_DATASETS, _BIO_DATASETS])]
  blacklist_str = ', '.join(blacklist) if blacklist else '""'

  # Input information into SQL template and perform the query.
  query_for_all_stat_vars = (QUERY_FOR_ALL_STAT_VARS
    .replace("{CONSTRAINTS}", constraint_string)
...
def generate_dependent_constraint_list():
  ...
  file is then read in and used to generate a pandas dataframe of depedent
  ...
  for spec in obs_spec_list.spec:
    ...
    # Get depedent variables which depend on the value of the constraint.
...
def remove_depedent_constraints(stat_vars):
  ...
  Returns: stat_vars with all dependent constraints imputed in place for all 
    rows.
  """
  # Generate list of depedent constraints from protobuf config.
  ...
  common_cols = (['measuredProp','populationType','statType'] + 
    ["orig_p" + x for x in range(MAX_CONSTRAINTS_WITH_DPV)])
  ...
  for c in range(1, MAX_CONSTRAINTS + 1):
    ...
    if f"orig_v{c}_x" in dpv_match and f"orig_v{c}_y" in dpv_match:
      ...
      stat_vars.loc[dpv_match.index, "numConstraints"] = (
        stat_vars.loc[dpv_match.index, "numConstraints"].apply(lambda x: x - 1))
  ...
  for c in range(1, MAX_CONSTRAINTS + 1):
    stat_vars = stat_vars.rename({f"orig_v{c}_x":f"orig_v{c}"}, axis=1)
  return stat_vars
  

def left_fill_columns(row):
  """ Removes holes in constraints after imputing depedent constraints. 
    Args:
  ...
  """
  row_constraints = min(row['numConstraints'], MAX_CONSTRAINTS_WITH_DPV)
  ...
  for base_col in range(1, row_constraints + 1):
    ...
    if pd.isna(row[f"p{base_col}"]):
      ...
      while search_location <= MAX_CONSTRAINTS:
...
def row_to_human_readable(row):
  ...
  # StatType (e.g. median) is prepending if the stat type is not measuredValue.
  stat_type = row['statType']
  if stat_type != "measuredValue" and stat_type != "Unknown":
    human_string = (capitalizeFirst(stat_type.replace("Value", ""))
                    + "_" + human_string)

  # Postpend renamed constraint fields.
  row_constraints = min(row['numConstraints'], MAX_CONSTRAINTS_WITH_DPV)
  for num in range(1, row_constraints + 1): 
    if not pd.isna(row[f"v{num}"]):
      human_string += "_" + row[f"v{num}"]

  # Postpend nmeasurement qualifier if it exists.
  ...
  # Postpend measurement denominator if it exists.
...
def ensure_no_overlapping_stat_vars(stat_vars):
  ...
  (e.g. different constraint, values, ect) that have the same name. There may
  ...
  overlapping_stat_var_names = (
    stat_vars[stat_vars.duplicated('HumanReadableName', keep=False)])
...
def build_original_constraints(row):
  ...
  while (next_constraint <= MAX_CONSTRAINTS
        and not pd.isna(row[f"orig_p{next_constraint}"])):
    constraints_text += f"{row[f'orig_p{next_constraint}']}:"
    constraints_text += f"dcs:{row[f'orig_v{next_constraint}']}\n"}:"
    current_constraint += 1
...
def row_to_stat_var_mcf(row):
  ...
  To integrate these new statistical variables into the datacommons graph,
  they need to uploaded as MCF nodes with a new dcid, but the same properties.
...
def remove_new_stat_vars(stat_vars, client):
  ...
  Pulls the exisiting StatVar list from production and makes sure that no new
  ...
  `google.com:datcom-store-dev.dc_kg_2020_07_12_01_01_43.Instance` WHERE type =
  "StatisticalVariable"
  """
...
def create_human_readable_names(stat_vars, client):
  """ Handles generating human readable statistical variables from BQ response.
  ...
  Args:
    ...
    client -> Authenticated BigQuery dataframe.
  ...
  stat_vars = remove_depedent_constraints(stat_vars)
...
def output_stat_var_documentation(stat_vars):
  ...
  # (False, True) -> Only group disaster statistical variables by populationType
  # if there are more than 1 statistical variables for that group.
  STAT_VAR_POPULATION_GROUPINGS.append(
    ("Disasters", natural_disasters, False, True))
  ...
  with open('statistical_variables.md', 'w', newline='') as f_out:
    # Output heading.
    f_out.write(DOCUMENTATION_BASE_TEXT)

    # Output each vertical group. Some verticals are always nested by population
    # nested_grouping is True and if nested_grouping is false then stat var
    # groups larger than 1 are grouped if condense_big_groups is true.
    for header, grouping, nested_grouping, condense_big_groups \
        in STAT_VAR_POPULATION_GROUPINGS:
      f_out.write(DOCUMENTATION_HEADER_START.replace("{HEADER}", header))

      for population_type in grouping:
        # GroupBy popType if nested or condensed and more one stat var.
        stat_vars_for_pop_type = stat_vars.query(
          f"populationType == '{population_type}'")['HumanReadableName']
        group_pop_type = (nested_grouping
            or (condense_big_groups and len(stat_vars_for_pop_type) > 1))

        if group_pop_type:
          f_out.write(DOCUMENTATION_DROPDOWN_START.replace("{POPULATION_TYPE}",
                      population_type))
        # Output individual statistical variable as a link to DC graph.
        for stat_var in stat_vars_for_pop_type:
          f_out.write(f"    <li><a href=\"https://browser.datacommons.org/kg?dcid={stat_var}\">{stat_var}</a></li>\n")
...
def main(argv):
  ...
  stat_vars = stat_vars.sort_values(
    ['populationType', 'numConstraints', 'statType', 'measuredProp',
  ...
  with open('renamed_stat_vars.mcf', 'w', newline='') as f_out:
    for _, row in stat_vars.iterrows():
      new_stat_var=row_to_stat_var_mcf(row)
      f_out.write(new_stat_var)

  # Output CSV for debugging.
  stat_vars=stat_vars.fillna("None")
  stat_vars[['numConstraints', 'populationType', 'measuredProp',
            'p1', 'v1', 'p2', 'v2', 'p3', 'v3', 'HumanReadableName',
            'orig_p1', 'orig_v1', 'orig_p2', 'orig_v2', 'orig_p3',
            'orig_v3', 'orig_p4', 'orig_v4']
