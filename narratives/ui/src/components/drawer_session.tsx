...
export function SessionDrawer() {
  ...
  // Resolve the section tabs the same way the header does. Reading NAV_CONFIG
  // directly ignored branding.json, so an instance that empties `navigation`
  // -- base DC does exactly that -- got no tabs in the header but still saw
  // them here.
  const { navigation } = useBrand();
        ...
        >
          ...
          <div className="w-1/2 h-full flex flex-col">
            ...
            {/* Primary section tabs — the same four entries as the header nav
                (resolveNavItems). The active tab gets a brand-primary dot + text.
          ...
          <div className="w-1/2 h-full flex flex-col">
            ...
            <div className="flex items-center gap-2 px-2 py-2 shrink-0">
              <Tooltip label="Back to menu" placement="bottom">
              <button
                type="button"
                aria-label="Back"
