...
spec = importlib.util.spec_from_file_location(
    "gcp_auth", "src/narratives_agent/gcp_auth.py"
)
ga = importlib.util.module_from_spec(spec)
spec.loader.exec_module(ga)
ga.get_id_token = lambda audience: (
    "fake-id-token"
)  # no metadata server in tests

CASES = [
    ("https://api.datacommons.org/mcp", "X-API-Key", "test-key-123"),
    ("https://datacommons.org/api", "X-API-Key", "test-key-123"),
    (
        "https://x-dc-datacommons-service-uc.a.run.app/mcp",
        "Authorization",
...
for url, header, expected in CASES:
    ...
    print(
        f"  {'ok  ' if ok else 'FAIL'} {url[:52]:54s} -> {headers or '{} (no auth)'}"
    )
    if not ok:
        fails.append(url)
...
print(
    f"  {'ok  ' if not leaked else 'FAIL'} API key NOT sent to an unlisted host"
)
if leaked:
    fails.append("api key leaked to unlisted host")

print(
    f"\n{len(CASES) + 1 - len(fails)}/{len(CASES) + 1} checks passed"
    if not fails
    else f"\nFAILURES: {fails}"
)
