...
check(
    "1.2.x populated  -> has_data",
    du.check_data_availability([tool_call("get_observations", V121_WITH_DATA)])[
        "has_data"
    ],
)
check(
    "1.2.x empty      -> no data",
    not du.check_data_availability([tool_call("get_observations", V121_EMPTY)])[
        "has_data"
    ],
)
check(
    "1.3.x populated  -> has_data",
    du.check_data_availability([tool_call("get_observations", V130_WITH_DATA)])[
        "has_data"
    ],
)
check(
    "1.3.x empty      -> no data  (the silent wrong-answer bug)",
    not du.check_data_availability([tool_call("get_observations", V130_EMPTY)])[
        "has_data"
    ],
)
...
check(
    "empty server -> generation 'unknown'",
    caps_mod.from_tools([]).generation == "unknown",
)
check(
    "no version string branching leaks out",
    c130.generation == "1.3.x-or-later" and c121.generation == "1.2.x",
)

print("\nargument coercion — must cover every generation's tool names:")
check(
    "date=range forced on get_observations",
    schema.fix_tool_arguments("get_observations", {"date_range_start": "2000"})[
        "date"
    ]
    == "range",
)
check(
    "date=range forced on get_child_observations (1.3.x only)",
    schema.fix_tool_arguments(
        "get_child_observations", {"date_range_start": "2000"}
    )["date"]
    == "range",
)
check(
    "places coerced to a list on search_child_indicators",
    schema.fix_tool_arguments("search_child_indicators", {"places": "India"})[
        "places"
    ]
    == ["India"],
)
check(
    "variable_dcids coerced to a list on get_variable_metadata",
    schema.fix_tool_arguments(
